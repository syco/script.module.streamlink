...
def parse_html(
    ...
    *args, **kwargs
    ...
    """Wrapper around ET.HTML with some extras.

    Provides these extra features:
     - Removes ET.XML declarations of invalid XHTML5 documents
     - Wraps errors in custom exception with a snippet of the data in the message
    """
    if isinstance(data, str) and data.lstrip().startswith("<?xml"):
        data = re.sub(r"^\s*<\?xml.+?\?>", "", data)

    return _parse(ET.HTML, data, name, exception, schema, *args, **kwargs)
...
def parse_xml(
    ...
    *args, **kwargs
    ...
    """Wrapper around ET.XML with some extras.

    Provides these extra features:
     - Handles incorrectly encoded ET.XML
    ...
    return _parse(ET.XML, data, name, exception, schema, *args, **kwargs)
